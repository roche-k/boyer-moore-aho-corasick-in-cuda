...
__global__ void ac_kernel1(int *d_state_transition,
                           unsigned int *d_state_supply,
                           unsigned int *d_state_final, unsigned char *d_text,
                           unsigned int *d_out, size_t pitch, int m, int n,
    ...
    if (blockIdx.x == numBlocks - 1 && threadIdx.x == blockDim.x - 1)
        ...
        stopThread = startThread + charactersPerThread + m - 1;
    ...
    for (column = startThread; (column < stopThread && column < n); column++) {
        ...
        while ((s = d_state_transition[r * effective_pitch + (d_text[column])]) ==
            -1)
        r = d_state_supply[r];
        r = s;
...
void cuda_ac1(int m, unsigned char *text, int n, int p_size, int alphabet,
              int *state_transition, unsigned int *state_supply,
    ...
    checkCudaErrors(cudaMalloc((void **)&d_state_supply,
                                (m * p_size + 1) * sizeof(unsigned int)));
    checkCudaErrors(cudaMalloc((void **)&d_state_final,
                                (m * p_size + 1) * sizeof(unsigned int)));
    checkCudaErrors(cudaMalloc((void **)&d_out, n * sizeof(unsigned int)));

    // Allocate 2D device memory
    checkCudaErrors(cudaMallocPitch(&d_state_transition, &pitch,
    ...
    checkCudaErrors(cudaMemcpy(d_state_supply, state_supply,
                                (m * p_size + 1) * sizeof(unsigned int),
                                cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(d_state_final, state_final,
                                (m * p_size + 1) * sizeof(unsigned int),
                                cudaMemcpyHostToDevice));
    ...
    checkCudaErrors(cudaMemcpy2D(d_state_transition, pitch, state_transition,
    ...
    ac_kernel1<<<dimGrid, dimBlock>>>(d_state_transition, d_state_supply,
                                        d_state_final, d_text, d_out, pitch, m, n,
                                        p_size, alphabet, numBlocks);
