...
__global__ void shared_kernel1(unsigned char *d_text, unsigned int *d_out,
                               int m, int n, int p_size, int alphabet,
                               int numBlocks, int sharedMemSize) {
  ...
  for (int globalMemIndex = blockIdx.x * sharedMemSize; globalMemIndex < n;
    ...
    /*if ( threadIdx.x == 0 )
        for ( i = globalMemIndex, j = 0; ( j < sharedMemSize + m - 1 && i < n );
       i++, j++ ) s_array[j] = d_text[i];
    */

    for (i = globalMemIndex + threadIdx.x, j = 0 + threadIdx.x;
         (j < sharedMemSize + m - 1 && i < n); i += blockDim.x, j += blockDim.x)
      s_array[j] = d_text[i];
    ...
    for (column = startThread;
         (column < stopThread && globalMemIndex + column < n); column++) {
...
void shared1(int m, unsigned char *text, int n, int p_size, int alphabet,
             int *go_to_function, unsigned int *failure_function,
             unsigned int *output_function) {
  ...
  checkCudaErrors(cudaMalloc((void **)&d_failure_function,
                             (m * p_size + 1) * sizeof(unsigned int)));
  checkCudaErrors(cudaMalloc((void **)&d_output_function,
                             (m * p_size + 1) * sizeof(unsigned int)));
  checkCudaErrors(cudaMalloc((void **)&d_out, n * sizeof(unsigned int)));

  // Allocate 2D device memory
  checkCudaErrors(cudaMallocPitch(&d_go_to_function, &pitch,
                                  alphabet * sizeof(int), (m * p_size + 1)));

  // Copy 1D host memory to device
  checkCudaErrors(cudaMemcpy(d_text, text, n * sizeof(unsigned char),
                             cudaMemcpyHostToDevice));
  checkCudaErrors(cudaMemcpy(d_failure_function, failure_function,
                             (m * p_size + 1) * sizeof(unsigned int),
                             cudaMemcpyHostToDevice));
  checkCudaErrors(cudaMemcpy(d_output_function, output_function,
                             (m * p_size + 1) * sizeof(unsigned int),
                             cudaMemcpyHostToDevice));
  checkCudaErrors(cudaMemcpy(d_out, h_out, n * sizeof(unsigned int),
                             cudaMemcpyHostToDevice));

  // Copy 2D host memory to device
  checkCudaErrors(cudaMemcpy2D(d_go_to_function, pitch, go_to_function,
                               alphabet * sizeof(int), alphabet * sizeof(int),
                               (m * p_size + 1), cudaMemcpyHostToDevice));

  // Bind the preprocessing tables to the texture cache
  cudaChannelFormatDesc desc = cudaCreateChannelDesc<int>();
  checkCudaErrors(cudaBindTexture2D(0, tex_go_to_function, d_go_to_function,
                                    desc, alphabet, m * p_size + 1, pitch));
  checkCudaErrors(cudaBindTexture(0, tex_failure_function, d_failure_function,
                                  (m * p_size + 1) * sizeof(unsigned int)));
  checkCudaErrors(cudaBindTexture(0, tex_output_function, d_output_function,
                                  (m * p_size + 1) * sizeof(unsigned int)));
  ...
  shared_kernel1<<<dimGrid, dimBlock, sharedMemSize + m - 1>>>(
      d_text, d_out, m, n, p_size, alphabet, numBlocks, sharedMemSize);
...
int main() {
  ...
  shared1(M, charText, text.size(), D, 256, goToTable, f, out);
  return 0;
}
